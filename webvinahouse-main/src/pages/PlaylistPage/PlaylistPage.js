...
import {useMusicPlayer} from "../../store/useMusicPlayer"; // File CSS riêng cho trang Playlist
...
function PlaylistPage() {
  ...
  useEffect(() => {
    ...
    const fetchPlaylistData = async () => {
      ...
      try {
        const response = await apiClient.get(`/get-playlists-by-id?id=${playlistId}`);
        // backend: API cần trả về object chứa thông tin playlist và mảng các bài hát (đầy đủ thông tin bài hát)
        if (response.data) {
          setPlaylistInfo(response.data); // Ví dụ: { name, description, cover_image_url, creator_name? }
