...
import {useMusicPlayer} from "../../store/useMusicPlayer"; // File CSS riêng
...
function GenrePage() {
  ...
  const fetchSongs = useCallback(async (pageNum = 1) => {
    ...
    try {
        console.log("name", genreSlug);
      const response = await apiClient.get(`/get-genres-by-name?name=` + genreSlug, {
        params: { page: pageNum, limit: 10 } // Ví dụ lấy 10 bài mỗi lần
      });
      const newSongs = response.data.songs.map(song => ({
          id: song.id,
          title: song.title,
